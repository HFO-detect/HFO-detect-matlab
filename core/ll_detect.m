...
function [df_out]=ll_detect(data, fs, low_fc, high_fc, threshold, window_size, window_overlap)
...
% LL = compute_hilbert_energy(filt_data);
% LL = compute_hilbert_envelope(filt_data);
% LL = compute_stenergy(filt_data, window_size * fs);
% LL = compute_teager_energy(filt_data);
% LL= compute_rms(filt_data, window_size*fs);
...
while win_start < length(filt_data)
    ...
    LL = [LL extract_line_lenght(filt_data(win_start:win_stop))];
    % LL = [LL extract_stenergy(filt_data(win_start:win_stop))];
    % LL = [LL extract_rms(filt_data(win_start:win_stop))];
...
%det_th = th_percentile(LL,threshold);
%det_th = th_quian(LL,threshold);
%det_th = th_turkey(LL,threshold);

%% Detect
LL_idx=1;
while LL_idx < length(LL)
    if LL(LL_idx) >= det_th
        event_start = LL_idx * samp_win_inc;
        while (LL_idx <= length(LL)) && (LL(LL_idx) >= det_th)
            LL_idx = LL_idx + 1;
        end
