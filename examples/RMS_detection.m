...
plot(data)
hold on
for i = 1:size(RMS_df.event_start,2)
    det_size = RMS_df.event_stop(i) - RMS_df.event_start(i);
    plot(linspace(double(RMS_df.event_start(i)),double(RMS_df.event_stop(i)),double(det_size)),...
         data(RMS_df.event_start(i)+1:RMS_df.event_stop(i)),'r')
end
hold off
